  ...
  class SessionsController < ApplicationController
    ...
    def saml_callback
      ...
      if @sso_service.persist_authentication!
        ...
        StatsD.increment(STATSD_SSO_CALLBACK_KEY, tags: ['status:success', "context:#{saml_response.context_key}"])
      else
        redirect_to url_service.login_redirect_url(auth: 'fail', code: @sso_service.auth_error_code)
        StatsD.increment(STATSD_SSO_CALLBACK_KEY, tags: ['status:failure', "context:#{saml_response.context_key}"])
